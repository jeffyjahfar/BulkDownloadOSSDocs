...
public class ManualDownloader {
    public static String BASE_URL="https://canonical.com";
    public static String DOCUMENTATION_DIR="./output/documentation/canonical/";
    ...
    private static void getPackagesList() {
        ...
        try {
            ...
            Elements elements = doc.select("div[id=products] div[class=p-card--strip u-no-padding] a");
            for(Element element: elements){
                try{
                    String url = element.attr("href");
                    String productName = sanitizeProductName(element.text());
                    System.out.println("*****"+productName+"*****");
                    String doctext = Jsoup.connect(BASE_URL+url).get().select("div[id=main-content").get(0).text();
